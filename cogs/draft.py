...
class PickUserView(discord.ui.View):
    ...
    async def _on_back(self, interaction: discord.Interaction):
        self.wizard.picks.pop()
        pick_number = len(self.wizard.picks) + 1
        view = PickUserView(self.wizard)
        await interaction.response.edit_message(
            content=f"**Pick {pick_number} of {self.wizard.team_count}** — who's up for this pick?",
            view=view,
        )
...
class EligibleTeamsWizard:
    ...
    def flat_selected_abbrs(self) -> list[str]:
        ...
        for conf in self.conferences:
            result.extend(self.per_conf_selections.get(conf, []))
        return result
    ...
    async def after_conference_selection(self, interaction: discord.Interaction, selected: list[str]):
        ...
        conference = self.current_conference()
        self.per_conf_selections[conference] = selected
        self.index += 1
...
class ConferenceTeamMultiSelectView(discord.ui.View):
    ...
    def __init__(self, wizard: EligibleTeamsWizard, conference: str, teams: list[dict], draft_min: int, draft_max: int,
        ...
        options = [
            discord.SelectOption(label=t["name"][:100], value=t["abbr"][:100], default=t["abbr"] in preselected_set)
            for t in teams[:25]
        ]
    ...
    async def _on_back(self, interaction: discord.Interaction):
        await self.wizard.go_back_from_team_select(interaction)
...
class EligibleConferenceSelectView(discord.ui.View):
    ...
    def __init__(self, preselected: list[str] | None = None):
        ...
        options = [
            discord.SelectOption(label=c[:100], value=c[:100], default=c in preselected_set)
            for c in conference_names[:25]
        ]
        select = discord.ui.Select(
...
class Draft(commands.Cog):
    ...
    @app_commands.command(
        name="switch_draft_pick",
        description="Change which team a user already drafted (admin only)",
    )
    @app_commands.describe(user="The user whose pick you're correcting", new_team="The team they should have instead")
    async def switch_draft_pick(self, interaction: discord.Interaction, user: discord.Member, new_team: str):
        if not is_admin(interaction):
            await send_ephemeral(interaction, "Only admins can do that.")
            return
        ...
        entry = next((e for e in order if e["user_id"] == user.id and e.get("picked_team")), None)
        if entry is None:
            await send_ephemeral(interaction, f"{user.mention} hasn't made a pick yet.")
            return

        old_abbr = entry["picked_team"]
        teams = load_teams()

        new_abbr, error = resolve_team(new_team, teams)
        if error:
            await send_ephemeral(interaction, error)
            return

        if new_abbr == old_abbr:
            await send_ephemeral(interaction, f"{user.mention} already has **{teams[new_abbr]['name']}**.")
            return

        roster = load_roster()
        if new_abbr in roster and roster[new_abbr]["user_id"] != user.id:
            owner_id = roster[new_abbr]["user_id"]
            await send_ephemeral(
                interaction,
                f"**{teams[new_abbr]['name']}** is already owned by <@{owner_id}>. "
                f"Vacate or switch that pick first.",
            )
            return
        ...
        if old_abbr in roster:
            del roster[old_abbr]
        roster[new_abbr] = {"user_id": user.id, "username": str(user)}
        save_roster(roster)

        entry["picked_team"] = new_abbr
        save_draft(draft)
    ...
    @switch_draft_pick.autocomplete("new_team")
    async def switch_draft_pick_autocomplete(self, interaction: discord.Interaction, current: str):
        teams = load_teams()
        current_lower = current.lower()
        matches = [
            t for abbr, t in teams.items()
